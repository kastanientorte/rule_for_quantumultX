...
( () => {
    ...
    if (!magicJS.isResponse) {
        magicJS.logWarning("非响应阶段触发，请检查脚本配置。");
        return magicJS.done();
    }

    const url = magicJS.request.url;
    const rawBody = magicJS.response.body;

    // ★ 防御：body 为空时直接放行
    if (!rawBody) {
        magicJS.logWarning("响应体为空，跳过处理。");
        return magicJS.done();
    }
    ...
    switch (true) {
        ...
        case /^https:\/\/app\.bilibili\.com\/x\/v2\/feed\/index\/story\?/.test(url):
            try {
                ...
                if (obj?.data?.items?.length > 0) {
                    ...
                    if (aid) magicJS.write(storyAidKey, aid);
                }
            } catch (err) {
        ...
        case /^https?:\/\/app\.bilibili\.com\/x\/resource\/show\/tab/.test(url):
            try {
                ...
                let storyAid = magicJS.read(storyAidKey) || "246834163";
                if (obj?.data?.top) {
        ...
        case /^https?:\/\/app\.bilibili\.com\/x\/v2\/feed\/index\/relate\/story\?/.test(url):
            try {
                ...
                magicJS.logError(`Story模式处理出现异常：${err}`);
            }
            break;
    ...
    // ★ Surge 兼容：显式传 headers 和 status，避免响应异常
    if (body) {
        magicJS.done({
            body: body,
            headers: magicJS.response.headers,
            status: magicJS.response.status
        });
    } else {
        magicJS.done();
    }
...
function MagicJS(scriptName = "MagicJS", logLevel = "INFO") {
    return new class {
        ...
        get response() {
            if ("undefined" == typeof $response) return void 0;
            // ★ Surge 兼容：不直接修改 $response，创建浅拷贝
            let resp = Object.assign({}, $response);
            if (resp.hasOwnProperty("status") && !resp.hasOwnProperty("statusCode")) {
                resp.statusCode = resp.status;
            }
            if (resp.hasOwnProperty("statusCode") && !resp.hasOwnProperty("status")) {
